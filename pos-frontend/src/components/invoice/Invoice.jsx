...
const Invoice = ({ orderInfo, setShowInvoice }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
      <div className="bg-white p-3 md:p-4 rounded-lg shadow-lg w-full max-w-[95%] sm:max-w-[400px]">
        {/* Receipt Content for Printing */}

        <div ref={invoiceRef} className="p-2 md:p-4">
          {/* Receipt Header */}
          <div className="flex justify-center mb-3 md:mb-4">
            <motion.div
              ...
              className="w-10 h-10 md:w-12 md:h-12 border-6 md:border-8 border-green-500 rounded-full flex items-center justify-center shadow-lg bg-green-500"
            >
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
          ...
          <h2 className="text-lg md:text-xl font-bold text-center mb-1 md:mb-2">Order Receipt</h2>
          <p className="text-gray-600 text-center text-xs md:text-sm">Thank you for your order!</p>

          {/* Order Details */}

          <div className="mt-3 md:mt-4 border-t pt-3 md:pt-4 text-xs md:text-sm text-gray-700">
            <p>
          ...
          <div className="mt-3 md:mt-4 border-t pt-3 md:pt-4">
            <h3 className="text-xs md:text-sm font-semibold">Items Ordered</h3>
              ...
              <ul className="text-xs md:text-sm text-gray-700">
                {orderInfo.items.map((item, index) => (
                  ...
                  >
                    ...
                    <span className="whitespace-nowrap">₹{(item.price && typeof item.price === 'number') ? item.price.toFixed(2) : "0.00"}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-xs md:text-sm text-gray-500">No items in this order</p>
            )}
          </div>

          {/* Bills Summary */}

          <div className="mt-3 md:mt-4 border-t pt-3 md:pt-4 text-xs md:text-sm">
            {orderInfo.bills ? (
              <>
                <p>
        ...
        <div className="flex justify-between mt-3 md:mt-4">
          <button
            onClick={handlePrint}
            className="text-blue-500 hover:underline text-xs px-3 py-1 md:px-4 md:py-2 rounded-lg"
          >
            Print Receipt
          </button>
          <button
            onClick={() => setShowInvoice(false)}
            className="text-red-500 hover:underline text-xs px-3 py-1 md:px-4 md:py-2 rounded-lg"
          >
            Close
          </button>
        </div>
